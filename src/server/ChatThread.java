		...
		try{
			while (true)
			{
				...
				switch (msg.getType()) {
					...
					case Message.Type.LOGIN_OK://成功登录，需要更换信息
						for (ChatThread cht : server.getClients())
						{
							if (cht.account.getAccount().equals(((Info)(msg.getContent())).getAccount()))
							{
								cht.socket = this.socket;
								break;
							}
						}
						break;
...
/*************************添加好友**************************/
	...
	{
		System.out.println("查询退出账号");
		ArrayList<ChatThread> cht = server.getClients();
		for (int i = 0; i < cht.size(); ++ i)
		{
			if (cht.get(i).account.getAccount().equals(msg.getSender().getAccount()))
			{
				System.out.println("查询到要退出的账号");
				Message sendMessage = new Message();
				sendMessage.setType(Message.Type.CLOSEOK);
				try {
				...
				try {
					cht.get(i).socket.close();
				} catch (IOException e1) {
					// TODO Auto-generated catch block
					e1.printStackTrace();
				}
				server.getClients().remove(cht.get(i));
				return;
			}
		}
	}
